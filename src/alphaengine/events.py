...
def data_dir() -> Path:
    ...
    gets committed. `XDG_DATA_HOME` is honoured on every platform, including
    Windows, so a test can redirect the dump without touching a real profile
    directory. `%LOCALAPPDATA%` remains the Windows default when XDG is unset.
    """
