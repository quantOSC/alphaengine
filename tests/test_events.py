...
def test_data_dir_honours_xdg_even_when_localappdata_is_set(tmp_path, monkeypatch):
    """Windows CI always has LOCALAPPDATA. XDG_DATA_HOME must still win or a
    test that redirects the dump lands it in the real profile instead of tmp."""
    from alphaengine.events import data_dir

    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path))
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path / "AppData" / "Local"))
    assert data_dir() == tmp_path / "alphaengine"
